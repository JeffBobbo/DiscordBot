...
sub new
{
  ...
  my $self = {
    ...
    argc => 0, # counter for argument_next
    last => 0, # counter for option_next
    flag => undef # flag for speed up on option_next
  };
  bless($self, $class);
  return $self;
}
...
sub argument_count
{
  return scalar(@{shift()->{arguments}});
}

sub argument_next
{
  my $self = shift();
  return $self->{arguments}->[$self->{argc}++];
}
...
sub option_next
{
  ...
  if (!$self->{flag} || $name ne $self->{flag})
  {
    $self->{flag} = $name;
    $self->{last} = 0;
  }

  for my $i (0..scalar(@{$self->{options}})-1)
  {
    my $opt = $self->{options}->[$i];
    next if ($opt->{name} ne $name);
    $self->{last} = $i+1;
    return $opt->{value};
  }
  return undef;
